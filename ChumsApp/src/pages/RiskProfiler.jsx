  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetchWithAuth('http://127.0.0.1:8000/api/risk-profile/', {
        ...
        body: JSON.stringify({
          ...profile,
          risk_band: profile.risk_tolerance, // deterministic band for backend context (ignored if not used)
  ...
  if (step === 3) {
    ...
    return (
      <div className="min-h-screen bg-gray-50 p-6">
        <div className="max-w-6xl mx-auto">
          {usedFallback && (
            <div className="mb-4 p-4 rounded-lg border border-yellow-300 bg-yellow-50 text-yellow-800 flex items-start">
              <AlertCircle className="w-5 h-5 mr-2 mt-0.5 flex-shrink-0" />
              <div>
                <p className="font-semibold">Using fallback recommendations</p>
                <p className="text-sm">We couldn’t reach the AI service right now. These recommendations are generated by our backup logic.</p>
              </div>
            </div>
          )}
          <button
