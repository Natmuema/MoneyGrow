...
def generate_ai_recommendations(profile):
    ...
    # Firm policy constraints to steer and validate the model output
    firm_policy = {
        "allowed_types": ["stocks","bonds","real_estate","money_market","mutual_funds","fixed_deposit","business","agriculture","digital_assets"],
        "bands": {
            "conservative": {"equity": [0, 30], "bonds": [60, 90], "cash": [0, 20]},
            "moderate": {"equity": [40, 65], "bonds": [30, 55], "cash": [0, 15]},
            "aggressive": {"equity": [70, 95], "bonds": [5, 25], "cash": [0, 10]}
        },
        "min_position_percent": 5,
        "max_single_position_percent": 40
    }

    # Compose structured user content for the model
    user_payload = {
        "profile": {
            "age": profile.age,
            ...
            "investment_timeline_months": profile.investment_timeline,
            "investment_timeline_years": profile.investment_timeline_years,
            "financial_goals": profile.financial_goals,
        },
        "constraints": firm_policy,
        "schema": {
            "required": ["name","type","minimum_amount","expected_return","risk_level","description","local_description","recommended_amount","rationale","confidence_score"]
        },
        "instructions": (
            "Return ONLY a JSON array of 3-5 recommendation objects. "
            "Values must be realistic for Kenya. "
            "Sum of recommended_amount must be <= investment_amount. "
            "type must be one of allowed_types. "
            "risk_level must match risk_tolerance. "
    ...
    try:
        response = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system", 
                    "content": "You are a financial education assistant for Kenyan retail investors. Return ONLY a valid JSON array; no prose."
                },
                {
                    "role": "user", 
        ...
        # Parse the AI response strictly as JSON array
        ...
        allowed_types = set(firm_policy["allowed_types"])
        expected_risk = profile.risk_tolerance
        
        for rec in recommendations:
            try:
                # Basic schema validations
                for key in ["name","type","minimum_amount","expected_return","risk_level","description","local_description","recommended_amount","rationale","confidence_score"]:
                    if key not in rec:
                        raise ValueError(f"Missing required field: {key}")

                # Field parsing
                name = str(rec["name"]).strip()[:200] or "Investment Option"
                inv_type = str(rec["type"]).strip()
                if inv_type not in allowed_types:
                    raise ValueError(f"Invalid type: {inv_type}")

                risk_level = str(rec["risk_level"]).strip().lower()
                if risk_level != expected_risk:
                    raise ValueError(f"Risk level mismatch: {risk_level} != {expected_risk}")

                minimum_amount = Decimal(str(rec["minimum_amount"]))
                expected_return = Decimal(str(rec["expected_return"]))
                recommended_amount = Decimal(str(rec["recommended_amount"]))
                confidence_score = float(rec["confidence_score"])

                if minimum_amount < 0 or expected_return < 0 or recommended_amount < 0:
                    raise ValueError("Negative numeric value not allowed")
                if not (0.0 <= confidence_score <= 1.0):
                    raise ValueError("confidence_score out of range 0-1")
                ...
                local_description = str(rec["local_description"]).strip()
                ...
                processed_rec = {
                    ...
                    'description': description or 'Investment opportunity',
                    'local_description': local_description or 'Fursa ya uwekezaji',
                    'recommended_amount': recommended_amount,
                    'rationale': rationale or 'AI-generated recommendation',
...
@api_view(['POST'])
@permission_classes([IsAuthenticated])
def chat_investment_advisor(request):
    """
    AI chat endpoint: answers user questions based on their saved risk profile
    and current active recommendations. Uses the same firm policy constraints.
    """
    try:
        if not request.user.is_authenticated:
            return Response({
                'error': 'Authentication required',
                'message': 'Please provide a valid JWT token to chat with AI'
            }, status=status.HTTP_401_UNAUTHORIZED)
        ...
        # Load user profile and active recommendations
        profile = get_object_or_404(RiskProfile, user=request.user)
        recs_qs = InvestmentRecommendation.objects.filter(
            risk_profile=profile, is_active=True
        ).select_related('investment').order_by('-confidence_score')
        ...
        recs_context = [{
            ...
            "expected_return": float(r.investment.expected_return),
        ...
        # If OpenAI unavailable, provide concise deterministic fallback
        if not client or not hasattr(settings, 'OPENAI_API_KEY'):
            top = recs_context[:3]
            if not top:
                return Response({
                    'response': "I don't have recommendations yet. Please generate your portfolio first, then ask me questions."
                }, status=status.HTTP_200_OK)
            bullets = "; ".join([f"{t['name']} ({t['type']}, ~{t['expected_return']}% p.a.)" for t in top])
            return Response({
                'response': f"Based on your {profile.risk_tolerance} profile and timeline of {profile.investment_timeline_years} years, your top picks are: {bullets}. Ask me about risk, returns, or how to start."
            }, status=status.HTTP_200_OK)

        system_msg = (
            "You are a financial education assistant for Kenyan retail investors. "
            "Answer concisely (3-6 sentences). Be specific to the provided profile and recommendations. "
            "Do NOT provide personalized financial advice; this is educational. "
            "Only reference allowed asset types if suggesting alternatives. "
            "If asked about products not in recommendations or allowed types, refuse and suggest an allowed alternative. "
            "Avoid generic platitudes; cite numbers from the given context when helpful."
        )
        ...
        reply = (ai.choices[0].message.content or "").strip()
        # Basic sanitization
        if not reply:
        ...
        logger.error(f"Error in chat_investment_advisor: {e}")
        return Response({
